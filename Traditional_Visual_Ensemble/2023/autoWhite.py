...
def white_balance_4(img):
    ...
    max_I_r_2, max_I_r, max_I_b_2, max_I_b, max_I_g = (
        int(r[0][0] ** 2),
        int(r[0][0]),
        int(b[0][0] ** 2),
        int(b[0][0]),
    ...
    [u_b, v_b] = np.matmul(
        np.linalg.inv([[sum_I_b_2, sum_I_b], [max_I_b_2, max_I_b]]), [sum_I_g, max_I_g]
    )
    [u_r, v_r] = np.matmul(
        np.linalg.inv([[sum_I_r_2, sum_I_r], [max_I_r_2, max_I_r]]), [sum_I_g, max_I_g]
    )
    # print(u_b, v_b, u_r, v_r)
    b0, g0, r0 = (
        np.zeros(b.shape, np.uint8),
        np.zeros(g.shape, np.uint8),
        np.zeros(r.shape, np.uint8),
    )
    for i in range(m):
        for j in range(n):
...
def white_balance_LUT(img):
    # 自定义LUT表
    LUT = np.zeros((256, 1, 3), dtype=np.uint8)

    LUT[:, 0, 0] = np.clip(np.arange(256) + 10, 0, 255)
    LUT[:, 0, 1] = np.clip(np.arange(256) - 15, 0, 255)
    LUT[:, 0, 2] = np.clip(np.arange(256) - 25, 0, 255)
